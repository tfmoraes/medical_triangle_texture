...
def parse_cmd_line():
    ...
    parser.add_option('--nslices', dest='nslices', type="int", default=5,
                      help=u'Number of slices')

    parser.add_option('--offset', dest='offset', type="int", default=5,
                      help=u'Offset in mm')
...
def main():
    ...
    #  cpdata = np.memmap('/tmp/doidos.dat', shape=dim, dtype='int16', mode='w+')
    #  cpdata[:] = 0
    vertices, normals, faces = read_ply(ifile)
    print vertices.shape, normals.shape, faces.shape
    tcoords, image, tnormals = gen_texture.generate_tcoords_hf(vertices.astype('float64'), normals.astype('float64'), faces.astype('int32'), tdata, spacing, ww, wl, clut, dim=dtexture, offset=offset, nslices=nslices)
    ...
    #  cvimg = converters.to_vtk(cpdata, spacing, 0, 'AXIAL')
    #  w = vtk.vtkXMLImageDataWriter()
    #  w.SetFileName('/tmp/manolo.vti')
    #  w.SetInput(cvimg)
    #  w.Write()
